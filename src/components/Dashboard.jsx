  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <main className="flex-1 flex overflow-hidden">
        {/* Left Panel: Chat */}
        <section className="w-full md:w-[400px] lg:w-[450px] border-r border-outline-variant/10 flex flex-col bg-surface-container-low/30 backdrop-blur-sm z-30">
          <header className="p-6 flex items-center justify-between z-20">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded flex items-center justify-center bg-surface-container-highest outline outline-1 outline-outline-variant/20 relative">
              ...
              <div>
                <h2 className="font-headline text-lg font-semibold tracking-tight text-on-surface">Aether Agent</h2>
          ...
          <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-6 custom-scrollbar">
            {messages.map((msg, i) => (
              <div key={i} className={`flex items-start gap-3 max-w-[90%] ${msg.role === 'user' ? 'self-end flex-row-reverse' : ''}`}>
                ...
                <div className={`p-4 outline outline-1 leading-relaxed text-sm ${msg.role === 'user' ? 'bg-surface-container-high rounded-l-xl rounded-br-xl outline-outline-variant/10 shadow-lg' : 'bg-surface-container rounded-r-xl rounded-bl-xl outline-outline-variant/15 text-on-surface/90 shadow-xl'}`}>
                  {msg.content}
                </div>
              </div>
            ))}
          ...
          <form onSubmit={handleSendMessage} className="p-6 pt-2">
            <div className="relative group">
              <input 
                ...
                className="w-full bg-surface-container-highest border border-outline-variant/20 rounded-xl py-4 pl-12 pr-4 text-sm font-body focus:outline-none focus:ring-2 focus:ring-primary/40 focus:border-primary/50 transition-all placeholder:text-on-surface-variant/40"
              />
              ...
              <button 
                ...
                className={`absolute right-4 top-1/2 -translate-y-1/2 p-2 rounded-lg transition-all ${isListening ? 'bg-primary/20 text-primary animate-pulse' : 'text-on-surface-variant/40 hover:text-primary'}`}
              >
        ...
        <section className="flex-1 p-8 overflow-y-auto flex flex-col gap-10 bg-[#0d0e0f]/20">
          <header className="flex justify-between items-end z-10">
            <div>
              ...
              <h1 className="font-headline text-5xl font-bold tracking-tighter text-on-surface">Neural Architect</h1>
            </div>
            <div className="flex flex-col items-end gap-2">
              ...
              <div className="glass-panel rounded-full px-4 py-2 flex items-center gap-3 border border-outline-variant/20 shadow-2xl">
                <div className="w-5 h-5 rounded-sm bg-surface-container-highest flex items-center justify-center">
                  <Table className="text-green-400" size={14} />
                </div>
                <span className="font-body text-xs text-on-surface font-medium">Sheets Link</span>
                <RefreshCw className={`text-primary ${isSyncing ? 'animate-spin' : ''}`} size={14} />
              </div>
              ...
              {roadmap && (
                ...
                >
                  ...
                  <span className="font-body text-xs text-on-surface font-medium">{isGeneratingFSD ? 'Drafting...' : 'Generate FSD'}</span>
                </button>
              )}
            </div>
          </header>
          ...
          <div className="flex flex-col lg:flex-row gap-8 min-h-[500px]">
            ...
            <div className="flex-1 bg-surface-container-lowest/50 rounded-[2rem] border border-outline-variant/10 relative p-8 flex flex-col z-10 overflow-hidden shadow-inset">
              <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,rgba(93,3,202,0.05)_0%,transparent_70%)] pointer-events-none"></div>
